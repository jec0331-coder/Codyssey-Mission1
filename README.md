...
- 2-1.트러블 슈팅 : 권한 변경할 폴더가 있는 디렉토리에 위치해 있지 않아서 에러발생함. 상위 디렉토리로 이동해 줌.
```bash
cd ..
```

- 2-2.트러블 슈팅 : 폴더의 권한 변경 명령어는 -R을 붙여야 하는데 실수로 -r을 붙여서 에러가남.
```bash
#에러난 코드
chmod -r 755 Mission1/
```
...
- 실무 핵심 권한(755 vs 644)의 구조 및 추천 사례
