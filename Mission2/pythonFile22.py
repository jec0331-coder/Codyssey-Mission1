...
def SelectAction(input):
    if input == 1:
        quizGame.QuizSolving()
    elif input == 2:
        quizGame.QuizCreating()
    elif input == 3:
        quizGame.ShowQuizList()
    elif input == 4:
        quizGame.ShowScoreRecord()
    elif input == 5:
        ProgramQuiting()


class QuizGame:
    def __init__(self):  # , quizList, highestScore, recordList):
        self.quizIndex = 1
        # 퀴즈 목록
        self.quizList = []  # quizList
        # 최고 점수;
        self.highestScore = 0  # highestScore
        # 기록 리스트
        self.recordList = []  # recordList
        self.remainingQuizIndexList = []
        self.newQuiz = Quiz("", "", "", "", "", "", 0)
    ...
    def PrintMenu(self):
        ...
        while True:
            ...
            try:
                # 입력 앞뒤 공백 제거;
                trimmedInput = int(input("선택:").strip())
                ...
                # 숫자인데; 범위 초과시;
                elif trimmedInput > 5 or trimmedInput < 1:
                    # 1~5사이의 숫자를 입력하세요;
                    print("1~5사이의 숫자를 입력하세요.")

                else:
                    SelectAction(trimmedInput)
                    break
            ...
            # 엑셉트; 키보드 인터럽트 에러;
                ...
                self.SaveDataToJsonFile()
                # 키보드 인터럽트 에러 발생! 데이터를 저장후 종료합니다.
                print("키보드 인터럽트 에러 발생! 데이터를 저장후 종료합니다.")
                break

            # 엑셉트; 입력 스트림 종료(EOFError);
            except EOFError:
                self.SaveDataToJsonFile()
                # 입력 스트림 종료(EOFError) 발생! 데이터를 저장후 종료합니다.
                print("입력 스트림 종료(EOFError) 발생! 데이터를 저장후 종료합니다.")
                break

    def QuizSolving(self):
        print("--------------------")
        print("선택: 1")
        print("")
        ...
        if len(self.quizList) == 0:
            ...
            print("퀴즈를 시작합니다! (총 ", len(self.quizList), "문제)")

            rQuiz = pickRandomRemainingQuiz()
            PrintQuizSolvingPage(rQuiz)

    def QuizCreating(self):
        print("--------------------")
        print("선택: 2")
        print("")
        print("새로운 퀴즈를 추가합니다.")
        ...
        while True:
            ...
            try:
                # 입력 앞뒤 공백 제거;
                trimmedInput = input("문제를 입력하세요: ").strip()
                # print("tr", trimmedInput)
                # 인풋값이 ""면;
                if trimmedInput == "":
                    # 다시 입력하세요;
                    # print("다시 입력하세요")
    ...
    def ShowQuizList(self):
        ## 테스트용 코드;
        # quizList = []
        print("--------------------")
        print("선택: 3")

        print("등록된 퀴즈 목록(총", len(self.quizList), "개)")
        print("--------------")
    ...
    def ShowScoreRecord(self):
        ## 테스트용 코드;
        # scoreRecordList = []
        print("--------------------")
        print("선택: 4")

        print("플레이 기록(총", len(self.recordList), "회)")
        print("--------------")
        ...
        if len(self.recordList) == 0:
            ...
            for record in self.recordList:
                ...
                print(
                    f"[{index}] 점수: {point} 점({len(self.quizList)}문제 중 {record.score}문제 정답), 날짜: {record.dateStr}"
                )
    ...
    def LoadDataFromJsonFile(self):
        try:
            # json파일 읽어들이기
            with open("Mission2/state.json", "r", encoding="utf-8") as file:
                loaded_data = json.load(file)

                #
                self.highestScore = loaded_data.get("best_score", 0)
                # print("sc", self.highestScore)
                # 퀴즈 리스트
                quizDictList = loaded_data.get("quizzes", [])
                # print("dl", len(quizDictList))

                self.quizList = [Quiz(**item) for item in quizDictList]
                # print("ql", len(self.quizList))
            ...
            #
            self.GenerateRemainingQuizIndexList()
            # 기록 리스트
            self.recordList = [Record(**item) for item in recordDictList]

    def SaveDataToJsonFile(self):
        quizDictlist = [vars(item) for item in self.quizList]

        recordDictlist = [vars(item) for item in self.recordList]

        # 두 리스트를 하나의 큰 딕셔너리로 묶기.
        combined_data = {
            "best_score": self.highestScore,
            "quizzes": quizDictlist,
            "recordData": recordDictlist,
        }

        # 파일 쓰기 (indent=4를 주면 가독성 있게 줄바꿈됩니다)
        with open("Mission2/state.json", "w", encoding="utf-8") as file:
            json.dump(combined_data, file, ensure_ascii=False, indent=4)

    def GenerateRemainingQuizIndexList(self):
        self.remainingQuizIndexList = []
        for i in range(len(self.quizList)):
            self.remainingQuizIndexList.append(i)

        # print("re", self.remainingQuizIndexList)
...
def pickRandomRemainingQuiz():
    ...
    rIndex = random.choice(quizGame.remainingQuizIndexList)
    rQuiz = quizGame.quizList[rIndex]
    quizGame.remainingQuizIndexList.remove(rIndex)
    return rQuiz
...
def PrintQuizSolvingPage(quiz):
    ...
    print("---------------------------")
    print("문제[" + str(quizGame.quizIndex) + "]")
    quiz.showQuiz()
    # print(quiz.question)
    # print("1. " + quiz.ex1)
    # print("2. " + quiz.ex2)
    ...
    while True:
        ...
        try:
            ...
            # 숫자인데; 범위 초과시;
            elif trimmedInput > 4 or trimmedInput < 1:
                # 1~4사이의 숫자를 입력하세요;
                print("1~4사이의 숫자를 입력하세요.")

            else:
                # selectAction(trimmedInput)
                decidedInput = trimmedInput
                break
    ...
    isCorrect = quiz.checkAnswer(decidedInput)

    if isCorrect == True:
        print("정답입니다!")
        quizGame.quizIndex += 1

        # 남은 문제가 있으면;
        if len(quizGame.remainingQuizIndexList) > 0:
            # 최고점수와 비교,갱신;
            solvedQuizCount = quizGame.quizIndex - 1
            if solvedQuizCount > quizGame.highestScore:
                quizGame.highestScore = solvedQuizCount
                quizGame.SaveDataToJsonFile()
...
def PrintScore():
    ...
    # 푼 문제수가 기록[0].스코어보다 크면;
    if len(quizGame.recordList) == 0:
        if solvedQuizCount > 0:
            # 최고 점수 획득!
            print("최고 점수 획득!")

    elif solvedQuizCount > quizGame.highestScore:  # scoreRecordList[0].score:
        quizGame.highestScore = solvedQuizCount
        print("최고 점수 획득!")

    print(
        len(quizGame.quizList), "문제 중", solvedQuizCount, "문제 정답!(", point, "점)"
    )
...
def AddNewRecordToRecordListAndSaveRecordList(score):
    ...
    tmpRecordList = []
    tmpRecordList.extend(quizGame.recordList)
    tmpRecordList.append(newRecord)

    sortedList = sorted(tmpRecordList, key=lambda x: x.score, reverse=True)

    quizGame.recordList.clear()
    quizGame.recordList.extend(sortedList)
    # saveRecordList()
    quizGame.SaveDataToJsonFile()
...
def SaveQuestion(inputStr):
    global quizGame
    # 새퀴즈.질문=문자열;
    quizGame.newQuiz.question = inputStr
    ...
    while True:
        ...
        try:
            # 입력 앞뒤 공백 제거;
            trimmedInput = input("선택지 1: ").strip()
            ...
            #    print("1~5사이의 숫자를 입력하세요.")
                ...
                # SaveQuestion(input(trimmedInput))
                SaveEx1(trimmedInput)
...
def SaveEx1(inputStr):
    ...
    while True:
        ...
        try:
            # 입력 앞뒤 공백 제거;
            trimmedInput = input("선택지 2: ").strip()
...
def SaveEx2(inputStr):
    ...
    while True:
        ...
        try:
            # 입력 앞뒤 공백 제거;
            trimmedInput = input("선택지 3: ").strip()
...
def SaveEx3(inputStr):
    ...
    while True:
        ...
        try:
            # 입력 앞뒤 공백 제거;
            trimmedInput = input("선택지 4: ").strip()
...
def SaveHint(inputStr):
    global quizGame
    # 새퀴즈.힌트=문자열;
    quizGame.newQuiz.hint = inputStr
    ...
    while True:
        ...
        try:
            # 입력 앞뒤 공백 제거;
            trimmedInput = int(input("정답 번호 (1-4): ").strip())
...
def SaveAnswer(inputStr):
    ...
    # 새퀴즈.정답=문자열;
    quizGame.newQuiz.answer = inputStr
    # 퀴즈 리스트에 새퀴즈 추가;
    quizGame.quizList.append(quizGame.newQuiz)
    print("quizList ", len(quizGame.quizList))
    # json으로 퀴즈리스트 저장;
    # saveQuizList()
    quizGame.SaveDataToJsonFile()
    print("퀴즈가 추가되었습니다!")
